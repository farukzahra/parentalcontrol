...
public static class WindowsSessionManager
{
    ...
    public static DateTime GetSessionStartTime()
    {
        try
        {
            ...
            var saved = Core.Security.ConfigurationManager.LoadSessionData();
            if (saved.HasValue && saved.Value > DateTime.Now.AddHours(-24))
            {
                // Se foi salvo e está dentro das últimas 24h, usar o valor salvo
                return saved.Value;
            }
