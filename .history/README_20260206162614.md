# Controle Parental Windows 11

Sistema de controle parental que monitora tempo de uso do PC e bloqueia/desloga automaticamente ao atingir o limite configurado.

## 📦 Instalação Rápida

### Opção 1: Instalador Automático (Recomendado)

1. **Baixe o instalador** da [página de releases](https://github.com/farukzahra/parentalcontrol/releases)
2. **Execute** `ParentalControl-Setup-1.0.0.exe` como Administrador
3. **Siga** o assistente de instalação
4. **Configure** usando o aplicativo no Menu Iniciar

### Opção 2: Instalação Manual (Sem Inno Setup)

```powershell
# Execute como Administrador
cd c:\repo\parentalControl
.\Install-Manual.ps1
```

### Opção 3: Build do Instalador (Desenvolvedores)

**Requisitos:**
- [Inno Setup 6](https://jrsoftware.org/isdl.php) instalado
- .NET 8 SDK

```powershell
# Compilar e criar instalador
.\Build-Installer.ps1

# O instalador será gerado em: .\Installer\ParentalControl-Setup-1.0.0.exe
```

## 🚀 Uso

1. **Abra** "Controle Parental" no Menu Iniciar
2. **Configure** o tempo limite desejado (15 min a 8 horas)
3. **Escolha** a ação: Bloquear tela ou Fazer logout
4. **Salve** a configuração (requer senha de admin)
5. O serviço começará a monitorar automaticamente!

## 🗑️ Desinstalação

### Via Instalador
- Painel de Controle → Programas → Desinstalar "Controle Parental"

### Manual
```powershell
# Execute como Administrador
.\Uninstall-Manual.ps1
```

## Status do Projeto

✅ **CONCLUÍDO**:
- Estrutura de solution .NET 8 com 4 projetos
- Biblioteca Core com modelos, comunicação (Named Pipes) e segurança
- Windows Service protegido que monitora tempo de sessão
- Aplicativo WPF de configuração completo
- Sistema de bloqueio automático via APIs do Windows
- Scripts de instalação/desinstalação
- Instalador profissional com Inno Setup

🚧 **PENDENTE**:
- Agente de notificações Toast
- Proteção avançada contra encerramento do serviço
- Histórico de uso e relatórios

## Arquitetura

```
ParentalControl/
├── ParentalControl.Core/           ✅ CONCLUÍDO
│   ├── Models/                     - TimeLimit, SessionInfo, ServiceMessage
│   ├── Communication/              - PipeServer, PipeClient (Named Pipes)
│   └── Security/                   - WindowsAuthenticator, SessionManager, ConfigManager
│
├── ParentalControl.Service/        ✅ CONCLUÍDO
│   └── ParentalControlWorker.cs    - Windows Service que monitora e aplica bloqueio
│
├── ParentalControl.ConfigApp/      ✅ CONCLUÍDO
│   └── Interface WPF para configuração
│
└── ParentalControl.NotificationAgent/  🚧 PENDENTE
    └── Agente para exibir Toast Notifications
```

## Funcionalidades Implementadas

### ✅ Biblioteca Core (ParentalControl.Core)

**Modelos**:
- `TimeLimit`: Configuração de tempo máximo, ação (Lock/Logout), avisos
- `SessionInfo`: Informações da sessão (usuário, tempo decorrido, tempo restante)
- `ServiceMessage`: Mensagens entre serviço e agente (avisos, status, tamper)

**Comunicação**:
- `PipeServer`: Servidor Named Pipe para enviar mensagens do serviço
- `PipeClient`: Cliente para receber mensagens no agente de notificações

**Segurança**:
- `WindowsAuthenticator`: Autenticação com senha Windows via LogonUser API
- `WindowsSessionManager`: Bloqueio de tela (LockWorkStation) e logout (ExitWindowsEx)
- `ConfigurationManager`: Armazenamento de configurações no Registry (HKLM)

### ✅ Windows Service (ParentalControl.Service)

- **Monitoramento contínuo** do tempo desde o login
- **Avisos progressivos** aos 15, 10 e 5 minutos restantes
- **Bloqueio/Logout automático** quando tempo expirar
- **Logging** no Event Log do Windows
- **Comunicação via Named Pipes** com agente de notificações
- **Proteção**: Roda como SYSTEM, configuração armazenada em HKLM

### ✅ Aplicativo de Configuração WPF (ParentalControl.ConfigApp)

- Interface gráfica moderna em português
- Slider para configurar tempo (15 min a 8 horas)
- Presets rápidos (30min, 1h, 2h, 3h, 4h)
- Status em tempo real da sessão
- Verificação de privilégios de administrador
- Botão de teste para validar bloqueio

## Próximos Passos

### 1. Agente de Notificações (ParentalControl.NotificationAgent) - Alta prioridade
- Aplicativo leve que roda na sessão do usuário
- Exibe Windows Toast Notifications
- Recebe mensagens do serviço via Named Pipes
- Inicia automaticamente com o Windows

### 2. Proteções Avançadas - Média prioridade
- ACLs mais restritivas no serviço
- Proteção contra Task Manager
- Monitoramento de tentativas de bypass

### 3. Funcionalidades Extras - Baixa prioridade
- Histórico de uso com gráficos
- Múltiplos perfis de usuário
- Horários permitidos por dia da semana
- Envio de notificações por email

## 📋 Requisitos de Sistema

- **SO:** Windows 10 ou 11 (64-bit)
- **Runtime:** .NET 8 Runtime (incluído no instalador)
- **Privilégios:** Administrador para instalação
- **Conta:** Filho deve ser usuário padrão (não Administrator)

## ⚙️ Configurações

- Windows 11 (compatível com Windows 10)
- .NET 8 Runtime
- Privilégios de Administrador para instalação
- Conta do filho deve ser **Usuário Padrão** (não Administrator)

## Configurações Atuais

As configurações são armazenadas no Registry em:
```
HKEY_LOCAL_MACHINE\SOFTWARE\ParentalControl
```

**Valores**:
- `MaxMinutes` (DWORD): Tempo máximo em minutos (padrão: 60)
- `IsEnabled` (DWORD): 1 = ativado, 0 = desativado (padrão: 1)
- `Action` (String): "Lock" ou "Logout" (padrão: "Lock")

Dados da sessão são armazenados em:
```
HKEY_LOCAL_MACHINE\SOFTWARE\ParentalControl\SessionData
```

## ⚙️ Configurações

As configurações são armazenadas no Registry em:
```
HKEY_LOCAL_MACHINE\SOFTWARE\ParentalControl
```

**Valores**:
- `MaxMinutes` (DWORD): Tempo máximo em minutos (padrão: 60)
- `IsEnabled` (DWORD): 1 = ativado, 0 = desativado (padrão: 1)
- `Action` (String): "Lock" ou "Logout" (padrão: "Lock")

Dados da sessão são armazenados em:
```
HKEY_LOCAL_MACHINE\SOFTWARE\ParentalControl\SessionData
```

## 🔨 Desenvolvimento

### Compilação

```powershell
# Compilar todos os projetos
dotnet build

# Compilar apenas o serviço
dotnet build ParentalControl.Service\ParentalControl.Service.csproj

# Compilar app de configuração
dotnet build ParentalControl.ConfigApp\ParentalControl.ConfigApp.csproj
```

### Testes Locais

O serviço pode ser executado como aplicativo console para testes:

```powershell
cd ParentalControl.Service
dotnet run
```

## 🔒 Segurança

### Proteções Implementadas
- ✅ Serviço roda como SYSTEM
- ✅ Configurações armazenadas em HKLM (requer admin para modificar)
- ✅ APIs nativas do Windows para bloqueio/logout
- ✅ Comunicação segura via Named Pipes locais
- ✅ ACLs do serviço (usuários comuns não podem parar)
- ✅ Logging de todas as ações no Event Log
- ✅ Auto-restart do serviço em caso de falha
- 🚧 Auto-reinício configurado
- 🚧 Event Log Source registrado

### Limitações Conhecidas
- ⚠️ Administrador pode sempre contornar as proteções
- ⚠️ Safe Mode permite bypass (mitigação: senha de BIOS)
- ⚠️ Acesso físico ao hardware permite bypass

## Logs

O serviço registra eventos no Event Log do Windows:
- **Application Log** → Source: "ParentalControl"
- Tipos de eventos: Informação, Aviso, Erro

Visualizar logs:
```powershell
# Event Viewer
eventvwr.msc

# PowerShell
Get-EventLog -LogName Application -Source ParentalControl -Newest 20
```

## Desenvolvimento

### Estrutura de Branches (Sugestão)
- `main` - versão estável
- `develop` - desenvolvimento ativo
- `feature/*` - novas funcionalidades

### Convenções de Código
- C# 12 com nullable reference types
- Async/await para operações I/O
- Logging estruturado com ILogger
- Comentários XML em classes públicas

## Contribuição

Para adicionar funcionalidades:
1. Adicionar modelos em `ParentalControl.Core/Models`
2. Implementar lógica no serviço em `ParentalControl.Service`
3. Criar UI no aplicativo de configuração
4. Testar com conta de usuário padrão

## Licença

Este projeto está licenciado sob **Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International (CC BY-NC-SA 4.0)**.

**Resumo:**
- ✅ **Uso livre** - Você pode usar gratuitamente
- ✅ **Modificar** - Pode alterar e melhorar o código
- ✅ **Distribuir** - Pode compartilhar com outras pessoas
- ✅ **Open Source** - Código aberto e transparente
- ❌ **Uso comercial proibido** - Não pode vender ou lucrar com este software
- ⚠️ **Atribuição obrigatória** - Deve dar crédito ao autor original
- ⚠️ **Mesma licença** - Modificações devem usar a mesma licença

Veja o arquivo [LICENSE](LICENSE) para mais detalhes ou acesse: https://creativecommons.org/licenses/by-nc-sa/4.0/

## Autor

Desenvolvido por **farukzahra** como sistema de controle parental para Windows 11

[![License: CC BY-NC-SA 4.0](https://img.shields.io/badge/License-CC%20BY--NC--SA%204.0-lightgrey.svg)](https://creativecommons.org/licenses/by-nc-sa/4.0/)
